...
public class SalaryRegistry {
    ...
    public void registerSalary(int memberId, BigDecimal salary) {
        ...
        SalaryHistory history =
                salaryHistoryMap.getOrDefault(memberId, new SalaryHistory(memberId));
    ...
    public BigDecimal getTotalSalaryBudget() {
        return salaryMap.values().stream().reduce(BigDecimal.ZERO, BigDecimal::add);
    }
    ...
    public BigDecimal getMinSalary() {
        return salaryMap.values().stream().min(BigDecimal::compareTo).orElse(BigDecimal.ZERO);
    }

    public BigDecimal getMaxSalary() {
        return salaryMap.values().stream().max(BigDecimal::compareTo).orElse(BigDecimal.ZERO);
    }
    ...
    public static class SalaryHistory {
        ...
        public BigDecimal getPreviousSalary() {
            return salaryRecords.size() > 1
                    ? salaryRecords.get(salaryRecords.size() - 2)
                    : BigDecimal.ZERO;
        }
        ...
        public List<SalaryRecord> getAllRecords() {
            ...
            for (int i = 0; i < salaryRecords.size(); i++) {
                records.add(new SalaryRecord(changeDates.get(i), salaryRecords.get(i)));
            }
            return records;
        }
